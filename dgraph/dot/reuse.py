"""Plan source-level alias reuse for semantic DOT IR.

This file decides which repeated IR subtrees should become Python variables.
It does not render code; it only computes alias-selection metadata.
"""
...
from dgraph.dot.naming import sanitize_name

if TYPE_CHECKING:
    from dgraph.dot.ir import DotIR, IRChild, IRContinuation, IRTree
...
def plan_source_reuse(ir: "DotIR") -> ReusePlan:
    ...
    for size, base_name, signature, tree in ordered:
        ...
        if parents and len(parents) == 1 and counts[signature] == counts[next(iter(parents))]:
            continue
        alias = sanitize_name(base_name, used_names)
